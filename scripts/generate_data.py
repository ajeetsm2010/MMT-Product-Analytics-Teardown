...
SEGMENTS = ["Solo Explorer", "Family/Group", "Business"]
DEVICES = ["Android", "iOS"]
...
# Approximate hotel attachment probability
# These will be validated later using SQL.
BASE_ATTACHMENT = {
    "Solo Explorer": 0.031,
    "Family/Group": 0.142,
    "Business": 0.080
...
# ============================================================
# 2. FACT_APP_EVENTS + FACT_BOOKINGS
# ============================================================
...
for user in users:
    ...
    for _ in range(num_sessions):
        ...
        session_id = make_id("SES", session_counter)
        session_counter += 1
        ...
        experiment_group = random.choice(["Control", "Variant"])
        ...
        # Around 75% of searches select a flight
        if random.random() > 0.25:
            selected_time = session_time + timedelta(
            ...
            # Most selected flights see the hotel widget
            if random.random() < 0.82:

                widget_time = selected_time + timedelta(
                ...
                if experiment_group == "Variant":
                    add_probability = base_probability + 0.024
                else:
                    add_probability = base_probability

                hotel_added = random.random() < add_probability

                if hotel_added:

                    add_time = widget_time + timedelta(
                    ...
                    checkout_time = add_time + timedelta(
                    ...
                    # Some flight-only bookings still complete
                    if random.random() < 0.55:

                        checkout_time = widget_time + timedelta(
...
# ============================================================
# 3. ADD REPEAT BOOKINGS FOR RETENTION ANALYSIS
# ============================================================
...
for user_id in existing_users:
    ...
    # Approximately 18% of bookers make a later booking
    if random.random() < 0.18:
        ...
        previous_date = datetime.fromisoformat(
            str(previous_booking["booking_timestamp"])
        )

        repeat_date = previous_date + timedelta(
